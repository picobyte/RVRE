...
class font:
    ...
    _config = {"Inconsolata-Regular": {
     ...
     def __init__(self, name="Inconsolata-Regular", size=30):
    ...
    def get_font_file(self, name=None):
        name = name or TextView.font['name']
        return TextView.fonts[name]['dir'] + "/" + name + ".ttf"

    @property
    def max_char_per_line(self, name=None, size=None):
        return font._config[name or self.name][str(size or self.size)][0]

    @property
    def max_lines_per_screen(self, name=None, size=None):
        return font._config[name or self.name][str(size or self.size)][1]
